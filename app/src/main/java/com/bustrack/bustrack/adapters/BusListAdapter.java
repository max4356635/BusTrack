...
public class BusListAdapter extends RecyclerView.Adapter<BusListHolders>{
    ...
    public void onBindViewHolder(final BusListHolders holder, final int position){
        ...
        holder.departureStop.setText(Html.fromHtml(busObjects.get(position).getDepartureStop()));
        holder.arriveTime.setText(Html.fromHtml(busObjects.get(position).getArriveTime()));
